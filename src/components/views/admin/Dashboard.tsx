...
type DashboardProperty = {
  ...
  rating: string;
  reviews: number;
  stats: { title: string; value: string; icon: React.ElementType; trend: string; }[];
  analytics: Record<string, { name: string; total: number; online: number; cancelled: number; }[]>;
  guests: { name: string; status: string; room: string; checkIn: string; checkOut: string; }[];
  revenue: Record<string, { label: string; current: number; max: number; }[]>;
  events: Record<string, DashboardEventItem[]>;
  floors: string[];
  roomsData: Record<string, { no: string; name: string; status: string; }[]>;
};
import { 
  AreaChart, 
  Area, 
...
const ALL_PROPERTIES: DashboardProperty[] = [
  {
    id: 1,
...
export function Dashboard({ setCurrentView }: DashboardProps) {
  ...
  const handleNextProperty = () => {
    ...
    setSelectedFloor(nextProperty.floors[0]);
    setSelectedDate('9'); // reset date just in case
  };
  
  const handlePrevProperty = () => {
    const prevIndex = (propertyIndex - 1 + properties.length) % properties.length;
    const prevProperty = properties[prevIndex];
    setPropertyIndex(prevIndex);
    setSelectedFloor(prevProperty.floors[0]);
    setSelectedDate('9');
  };

  const handleAddEvent = (event: { title: string; time: string; color: string; bg: string; location?: string; description?: string }) => {
    setProperties(prevProperties => prevProperties.map((property, index) => {
      if (index !== propertyIndex) return property;

      const existingEvents = (property.events as Record<string, Array<{ title: string; time: string; color: string; bg: string }>>)[selectedDate] ?? [];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      {/* Hero Banner Section */}
      <div className="relative rounded-2xl overflow-hidden h-72 group shadow-lg border border-gray-200">
        <img 
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-6 md:p-10">
          <div>
            <h2 className="text-3xl md:text-4xl font-bold font-display text-white mb-2 tracking-tight">Experience Unrivaled</h2>
            <p className="text-2xl md:text-3xl font-display text-[#689249] font-semibold">Luxury & Comfort</p>
            <p className="text-white mt-4 text-sm md:text-base max-w-xl leading-relaxed font-medium">A warm welcome to {currentProperty.name}. Indulge in premium hospitality, exquisite dining, and unforgettable moments.</p>
          </div>
          
          <div className="flex items-center gap-3 flex-wrap">
            <button 
              onClick={() => setIsAddBookingModalOpen(true)}
              className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#689249] text-white rounded-lg hover:bg-[#557A3A] font-semibold text-sm transition-all shadow-lg"
            >
              <Plus size={16} />
              New booking
            </button>
            <button 
              className="flex items-center justify-center gap-2 px-5 py-2.5 border-2 border-white text-white rounded-lg hover:bg-white/10 font-semibold text-sm transition-all"
            >
              <Video size={16} />
              360° Hotel Tour
            </button>
        ...
        {/* Navigation arrows removed - static image */}
      </div>

      {/* Top Controls */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 py-2 max-w-7xl mx-auto">
        <div className="flex items-center gap-4 w-full sm:w-auto">
          <div>
            <h1 className="text-2xl font-bold font-display text-gray-900 tracking-tight">Dashboard Overview</h1>
            <p className="text-gray-500 mt-1 text-sm font-medium">Welcome back! Here's what's happening today at {currentProperty.name}.</p>
          </div>
        </div>
        <div className="w-full sm:w-auto flex justify-end">
          <span className="font-semibold text-gray-700 text-sm">June 9, 2024</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-3 flex flex-col gap-4">
          <div className="flex-1 rounded-2xl overflow-hidden shadow-sm border border-gray-100 relative bg-gray-200 group min-h-[160px]">
            <img 
            ...
            <div className="absolute inset-y-0 left-2 right-2 flex justify-between items-center text-white pointer-events-none">
              <button 
                onClick={handlePrevProperty}
                className="p-1.5 rounded-full bg-black/30 hover:bg-black/50 backdrop-blur-sm transition-colors pointer-events-auto"
              >
                <ChevronLeft size={16} />
              </button>
              <button 
            ...
            <div className="absolute bottom-3 left-0 right-0 px-3 flex justify-center items-center text-white pointer-events-none">
              <span className="text-[10px] font-bold truncate px-2 text-center drop-shadow-md">
        ...
        {/* Quick Actions (From Previous Dashboard) */}
        <Card className="lg:col-span-12 rounded-2xl shadow-sm border-gray-100">
          <CardHeader className="py-4">
            <CardTitle>Quick Actions</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-4">
            {[
              { label: 'Add New Property', icon: Building2, view: 'properties' },
              { label: 'Manage 360° Tours', icon: Star, view: 'tours' },
              { label: 'Create Promotion', icon: TrendingUp, view: 'marketing' },
              { label: 'Add Staff Member', icon: Users, view: 'staff' },
            ].map((action, i) => {
              ...
              return (
                <button 
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-[#689249]/30 hover:bg-[#689249]/5 transition-all text-left group"
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center bg-white border border-gray-100 shadow-sm group-hover:border-[#689249]/20 group-hover:text-[#689249]`}>
                    <Icon size={18} className="text-gray-500 group-hover:text-[#689249] transition-colors" />
                  </div>
        ...
        <Card className="lg:col-span-3 rounded-2xl shadow-sm border-gray-100 flex flex-col">
          ...
          <CardContent className="flex-1 flex flex-col">
            ...
            <div className="space-y-3 mt-auto">
              {currentProperty.events[selectedDate as keyof typeof currentProperty.events] ? (
                currentProperty.events[selectedDate as keyof typeof currentProperty.events].map((event, i) => (
