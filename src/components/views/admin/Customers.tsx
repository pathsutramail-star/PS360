...
import { Search, Filter, Download, MoreHorizontal, Mail, Phone, Star, Eye, Gift, X } from 'lucide-react';
...
export function Customers() {
  ...
  const handleAddPoints = (customerId: string) => {
    setCustomers(customers.map((customer) =>
      customer.id === customerId ? { ...customer, points: customer.points + 100 } : customer
    ));
    notify('Added 100 loyalty points', 'success');
  };
              ...
              {customers.map((customer) => (
                <tr key={customer.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="relative inline-block" ref={activeMenuId === customer.id ? dropdownRef : null}>
                      <button 
                      ...
                      {activeMenuId === customer.id && (
                        <div className="absolute right-0 mt-2 w-44 rounded-xl border border-gray-100 bg-white p-1 shadow-xl z-10">
                          <button onClick={() => { setActiveMenuId(null); setSelectedCustomer(customer); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50">
                            <Eye size={14} className="text-[#689249]" /> View profile
                          </button>
                          <button onClick={() => { setActiveMenuId(null); notify(`Email sent to ${customer.name}`, 'success'); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50">
                            <Mail size={14} className="text-gray-500" /> Send email
                          </button>
                          <button onClick={() => { setActiveMenuId(null); handleAddPoints(customer.id); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50">
                            <Gift size={14} className="text-amber-500" /> Add points
                          </button>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-[#689249]">Customer Profile</p>
                <h3 className="text-xl font-bold text-gray-900">{selectedCustomer.name}</h3>
              </div>
              <button onClick={() => setSelectedCustomer(null)} className="rounded-full p-2 text-gray-500 hover:bg-gray-100">
                <X size={18} />
              </button>
            </div>
            <div className="mt-6 space-y-3 text-sm text-gray-600">
              <div className="flex justify-between"><span>Email</span><span className="font-semibold text-gray-900">{selectedCustomer.email}</span></div>
              <div className="flex justify-between"><span>Phone</span><span className="font-semibold text-gray-900">{selectedCustomer.phone}</span></div>
              <div className="flex justify-between"><span>Bookings</span><span className="font-semibold text-gray-900">{selectedCustomer.bookings}</span></div>
              <div className="flex justify-between"><span>Loyalty Points</span><span className="font-semibold text-gray-900">{selectedCustomer.points}</span></div>
              <div className="flex justify-between"><span>Status</span><span className="font-semibold text-gray-900">{selectedCustomer.status}</span></div>
            </div>
            <div className="mt-6 flex justify-end">
              <button onClick={() => setSelectedCustomer(null)} className="rounded-lg bg-[#689249] px-4 py-2 text-sm font-semibold text-white hover:bg-[#557A3A]">
                Close
              </button>
            </div>
          </div>
